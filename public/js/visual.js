...
const CUSTOM_BG_DB = 'claudio-custom-background';
const CUSTOM_BG_STORE = 'assets';
const CUSTOM_BG_KEY = 'active-background';
const BG_SETTINGS_KEY = 'claudio-bg-settings-v1';
...
const bgSettings = { ...BG_SETTINGS_DEFAULTS };

let customBgImageEl = null;
let customBgVideoEl = null;
let customBgShadeEl = null;
let bgUploadInputEl = null;
let customBgObjectUrl = null;
let bgStudioEl = null;
let bgStudioPanelEl = null;
let bgStudioFabEl = null;
let bgStudioCloseEl = null;
let bgStudioUploadBtnEl = null;
let bgStudioResetBtnEl = null;
...
function loadBackgroundSettings() {
  try {
    ...
    for (const [key, fallback] of Object.entries(BG_SETTINGS_DEFAULTS)) {
      const value = Number(parsed[key]);
      bgSettings[key] = Number.isFinite(value) ? value : fallback;
    }
  } catch {}
}
...
function formatBackgroundSetting(key, value) {
  switch (key) {
    case 'blur': return `${Math.round(value)}px`;
    case 'brightness':
    case 'contrast':
    case 'saturation': return value.toFixed(2);
    case 'zoom': return `${Math.round(value * 100)}%`;
    case 'shade':
    case 'noise': return `${Math.round(value * 100)}%`;
...
function applyBackgroundSettings() {
  ...
  root.setProperty('--bg-filter-blur', `${bgSettings.blur}px`);
  root.setProperty('--bg-filter-brightness', `${bgSettings.brightness}`);
  root.setProperty('--bg-filter-contrast', `${bgSettings.contrast}`);
  root.setProperty('--bg-filter-saturation', `${bgSettings.saturation}`);
  root.setProperty('--bg-filter-zoom', `${bgSettings.zoom}`);
  root.setProperty('--bg-shade-opacity', `${bgSettings.shade}`);
  root.setProperty('--bg-noise-opacity', `${bgSettings.noise}`);
}

function syncBackgroundStudioUI() {
  const controls = [
    ['blur', 'bgBlurSlider', 'bgBlurValue'],
    ['brightness', 'bgBrightnessSlider', 'bgBrightnessValue'],
    ['contrast', 'bgContrastSlider', 'bgContrastValue'],
    ['saturation', 'bgSaturationSlider', 'bgSaturationValue'],
    ['zoom', 'bgZoomSlider', 'bgZoomValue'],
    ['shade', 'bgShadeSlider', 'bgShadeValue'],
    ['noise', 'bgNoiseSlider', 'bgNoiseValue'],
  ];

  for (const [key, sliderId, valueId] of controls) {
    const input = $(sliderId);
    const valueEl = $(valueId);
    if (!input || !valueEl) continue;
    input.value = String(bgSettings[key]);
    valueEl.textContent = formatBackgroundSetting(key, bgSettings[key]);
    updateRangeFill(input);
  }
}

export function openBackgroundStudio(options = {}) {
  if (!bgStudioEl) return;
  bgStudioEl.classList.add('is-open');
  if (options.pickMedia) {
    openBackgroundPicker();
  }
}

export function closeBackgroundStudio() {
  bgStudioEl?.classList.remove('is-open');
}

function initBackgroundStudio() {
  bgStudioEl = $('bgStudio');
  bgStudioPanelEl = $('bgStudioPanel');
  bgStudioFabEl = $('bgStudioFab');
  bgStudioCloseEl = $('bgStudioClose');
  bgStudioUploadBtnEl = $('bgStudioUploadBtn');
  bgStudioResetBtnEl = $('bgStudioResetBtn');

  if (!bgStudioEl || !bgStudioPanelEl || !bgStudioFabEl || !bgStudioCloseEl) return;

  bgStudioFabEl.addEventListener('click', () => {
    bgStudioEl.classList.toggle('is-open');
  });

  bgStudioCloseEl.addEventListener('click', () => {
    closeBackgroundStudio();
  });

  document.addEventListener('click', (event) => {
    if (!bgStudioEl.classList.contains('is-open')) return;
    if (bgStudioEl.contains(event.target)) return;
    closeBackgroundStudio();
  });

  document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape') closeBackgroundStudio();
  });

  bgStudioUploadBtnEl?.addEventListener('click', () => openBackgroundPicker());
  bgStudioResetBtnEl?.addEventListener('click', async () => {
  ...
  document.querySelectorAll('[data-bg-setting]').forEach((input) => {
    input.addEventListener('input', (event) => {
      const key = event.target.dataset.bgSetting;
      const nextValue = Number(event.target.value);
      if (!(key in bgSettings) || !Number.isFinite(nextValue)) return;
      bgSettings[key] = nextValue;
      applyBackgroundSettings();
      syncBackgroundStudioUI();
      saveBackgroundSettings();
    });
  });
}

async function openBackgroundDb() {
...
async function writeBackgroundBlob(blob) {
  ...
  await new Promise((resolve, reject) => {
    ...
    tx.objectStore(CUSTOM_BG_STORE).put(blob, CUSTOM_BG_KEY);
...
function setCustomBackgroundMode(active) {
  document.body.classList.toggle('has-custom-bg', active);
  customBgShadeEl?.classList.toggle('is-hidden', !active);
}

function revokeCustomBackgroundUrl() {
  if (!customBgObjectUrl) return;
  URL.revokeObjectURL(customBgObjectUrl);
  customBgObjectUrl = null;
}

function hideCustomBackgroundElements() {
  customBgImageEl?.classList.add('is-hidden');
  customBgVideoEl?.classList.add('is-hidden');
  if (customBgVideoEl) {
    customBgVideoEl.pause();
    customBgVideoEl.removeAttribute('src');
    customBgVideoEl.load();
  }
  if (customBgImageEl) customBgImageEl.style.backgroundImage = '';
}

async function applyCustomBackgroundBlob(blob) {
  if (!customBgImageEl || !customBgVideoEl || !customBgShadeEl) return;

  revokeCustomBackgroundUrl();
  hideCustomBackgroundElements();

  if (!blob) {
    setCustomBackgroundMode(false);
    return;
  }
  ...
  if (blob.type.startsWith('video/')) {
    await new Promise((resolve, reject) => {
      customBgVideoEl.onloadeddata = async () => {
        customBgVideoEl.onloadeddata = null;
        customBgVideoEl.onerror = null;
        setCustomBackgroundMode(true);
        customBgVideoEl.classList.remove('is-hidden');
        try { await customBgVideoEl.play(); } catch {}
        resolve();
      };
      customBgVideoEl.onerror = () => {
        customBgVideoEl.onloadeddata = null;
        customBgVideoEl.onerror = null;
        reject(new Error('video load failed'));
      };
      customBgVideoEl.src = customBgObjectUrl;
    }).catch(async () => {
      revokeCustomBackgroundUrl();
      hideCustomBackgroundElements();
      setCustomBackgroundMode(false);
      await clearBackgroundBlob().catch(() => {});
      window.showToast?.('该视频背景无法加载，已恢复默认背景');
    });
    return;
  }
  ...
  await new Promise((resolve, reject) => {
    ...
    image.onload = () => {
      setCustomBackgroundMode(true);
      customBgImageEl.style.backgroundImage = `url("${customBgObjectUrl}")`;
      customBgImageEl.classList.remove('is-hidden');
      resolve();
    };
    image.onerror = () => reject(new Error('image load failed'));
    image.src = customBgObjectUrl;
  }).catch(async () => {
    revokeCustomBackgroundUrl();
    hideCustomBackgroundElements();
    setCustomBackgroundMode(false);
    await clearBackgroundBlob().catch(() => {});
    window.showToast?.('该图片背景无法加载，已恢复默认背景');
  });
}

export function openBackgroundPicker() {
  bgUploadInputEl?.click();
}

export async function resetCustomBackground() {
  await applyCustomBackgroundBlob(null);
  await clearBackgroundBlob().catch(() => {});
}

export async function initCustomBackground() {
  customBgImageEl = $('customBgImage');
  customBgVideoEl = $('customBgVideo');
  customBgShadeEl = $('customBgShade');
  bgUploadInputEl = $('bgUploadInput');

  if (!customBgImageEl || !customBgVideoEl || !customBgShadeEl || !bgUploadInputEl) return;
  loadBackgroundSettings();
  applyBackgroundSettings();
  initBackgroundStudio();
  syncBackgroundStudioUI();

  bgUploadInputEl.addEventListener('change', async (event) => {
    const file = event.target.files?.[0];
    if (!file) return;
    await applyCustomBackgroundBlob(file);
    await writeBackgroundBlob(file).catch(() => {});
    event.target.value = '';
    window.showToast?.('背景已更新');
    openBackgroundStudio();
  });

  const savedBlob = await readBackgroundBlob().catch(() => null);
  if (savedBlob) {
    await applyCustomBackgroundBlob(savedBlob);
  } else {
    setCustomBackgroundMode(false);
  }
}
...
function initBorderGlow() {
  const existingCanvas = document.querySelector('.border-glow-canvas');
